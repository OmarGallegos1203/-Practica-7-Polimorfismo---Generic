 ...
 * RCLASE HIJA "HELICOPTERO"
 * @since 17/03/2022
 * @author Omar Gallegos
 * OMAR ESAU GALLEGOS MORENO 1969869
 */
public class Helicoptero extends Transporte implements ISElectrico, ISFrenos{
    //  M E T O D O  C O N S T R U C T O R
    public Helicoptero(int velocidad, int gas){
        super(velocidad, gas);
    }
    //  M E T O D O S
    public void onHelice(int gas){
        if(gas<155){
            System.out.println("NO ES POSIBLE ENCENDER HELICES!");
        }else{
            System.out.println("HELICE ENCENDIDA!");
        }
    }
